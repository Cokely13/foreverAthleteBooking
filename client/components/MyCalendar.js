...
const MyCalendar = () => {
  ...
  const handleConfirmSession = () => {
    ...
    alert(`Session requested for ${selectedSlot}`);
    setSelectedSlot(null);
    setReload(!reload); // Set the reload state variable to true to force a re-render of the ConfirmSessions component
  };
  ...
  return (
    <div>
      ...
      {id == 3 ? <ConfirmSessions numConfirmations={numConfirmations} setNumConfirmations={setNumConfirmations} /> : <div></div>}
    </div>
  );
  }
