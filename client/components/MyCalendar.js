...
import Modal from 'react-modal';
import { fetchResults, deleteResult, updateSingleResult } from '../store/allResultsStore';
import { fetchSingleUser } from '../store/singleUserStore';
    ...
    <div>
      ...
      <div style={{marginLeft: "35px", marginBottom: "35px"}}>
        <select onChange={handleChange} name="filterEvents" className='custom-select'>
          ...
          {results.map((({ event }) => event)).filter((item, i, ar) => ar.indexOf(item) === i).map
          ((result) => <option key={result} value={result}>{result}</option>)}
  ...
  {showModal && (
    <Modal isOpen={showModal} onRequestClose={() => setShowModal(false)}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <label>Event:</label>
          <input type="text" value={formData.event} onChange={(event) => setFormData({ ...formData, event: event.target.value })} required />
        </div>
        <div>
          <label>Duration:</label>
          <input type="text" value={formData.duration} onChange={(event) => setFormData({ ...formData, duration: event.target.value })} required />
        </div>
        <div>
          <label>User ID:</label>
          <input type="text" value={formData.userId} onChange={(event) => setFormData({ ...formData, userId: event.target.value })} required />
        </div>
